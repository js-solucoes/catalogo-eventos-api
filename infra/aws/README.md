...
4. **[`github-oidc-iam/`](./github-oidc-iam/)** — *(opcional, entrega ao cliente)* OIDC GitHub + role IAM + `ReadOnlyAccess` + lock DynamoDB + política ECR/ECS; reduz cliques no console. Ver também [`docs/deployment/github-oidc-aws.md`](../../docs/deployment/github-oidc-aws.md).
5. **[`aurora-phase2/`](./aurora-phase2/)** — cluster **Aurora MySQL** (sem RDS Proxy), SG + Secrets; reutiliza VPC/subnets do foundation.
6. **Fase 3 (publicação)** — container Docker, ECR, ECS com imagem da API, smoke no ALB: [`docs/deployment/phase3-ecs-publish.md`](../../docs/deployment/phase3-ecs-publish.md).
